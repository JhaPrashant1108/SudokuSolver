...
class imagepreprocessor:
    ...
    def extractSudoku(self):
        return self.parseGrid()

    def parseGrid(self):
        original = cv2.imread(self.imagePath,cv2.IMREAD_GRAYSCALE)
        processed = self.preProcessImage(original)
        corners = self.findCorners(processed)
        cropped = self.cropAndWarp(original,corners)
        squares = self.inferGrid(cropped)
        digits = self.getDigits(cropped,squares,28)
        finalImage = self.showDigits(digits)
        return finalImage

    def preProcessImage(self,img,skipDilate=False):
    ...
    def cropAndWarp(self,img, crop_rect):
    ...
    def getDigits(self,img, squares, size):
        digits = []
        img = self.preProcessImage(img.copy(), skipDilate=True)
    ...
    def showDigits(self,digits, colour=255):
        rows = []
...
if __name__ == "__main__":
    path = "C:\\Users\\Prashant\\Documents\\SudokuSolver\\dataset\\sudoku\\original\\image1000.original.jpg"
